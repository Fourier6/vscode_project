/*
 * nosaydie_pidcontrol.c
 *
 *  Created on: 2021年11月12日
 *      Author: Administrator
 */
#include "nosaydie_pidcontrol.h"
#include "headfile.h"
#include "common.h"
struct PID Left_ctr;            //左轮控制相关
struct PID Right_ctr;           //右轮控制相关
struct PID_moshi l_pid_location;  //位置式PID
struct PID_moshi l_pid_speed;     //速度PID
struct PID_moshi r_pid_location;  //位置式PID
struct PID_moshi r_pid_speed;     //速度PID
struct PID_moshi ServoTurn;           //舵机控制PID
//p i d初始化
void pid_init()
{
    //左轮p i d初始化
