...
//  @note       图像的宽度如果超过液晶的宽度，则自动进行缩放显示。这样可以显示全视野
//-------------------------------------------------------------------------------------------------------------------
extern uint8 image[120][160];
extern struct LEFT_EDGE L_edge[150];     //左边界结构体
extern struct RIGHT_EDGE R_edge[150];    //右边界结构体
extern uint8 L_search_edge_count;
extern uint8 R_search_edge_count;
...
void binImage_display(uint8 width, uint8 height)
{
    uint16 coord_x = 0;
    uint16 coord_y = 0;
    coord_x = width>TFT_X_MAX?TFT_X_MAX:width;
    coord_y = height>TFT_Y_MAX?TFT_Y_MAX:height;
    lcd_set_region(0,0,coord_x-1,coord_y-1);
    for(int row=0;row<coord_y;row++)
    {
        for(int col=0;col<coord_x;col++)
        {
            if(image[row][col] == 0) lcd_writedata_16bit(BLACK);        //如果是0，tft屏幕显示黑
            else if(image[row][col] == 1) lcd_writedata_16bit(WHITE);   //如果是1，tft屏幕显示白
        }
    }
